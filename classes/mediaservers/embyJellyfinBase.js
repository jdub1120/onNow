...
class EmbyJellyfinBase {
  ...
  _mediaBrowserAuthorizationHeader(key) {
    ...
    return `MediaBrowser Client="OnNow", Device="OnNow", DeviceId="onnow", Version="${ver}", Token="${tok}"`;
  }
  ...
  async apiGet(path, options = {}) {
    ...
    if (!key) {
      throw new Error(
        `${this.appName}: no API key in settings. Create one in the server dashboard (Jellyfin: Dashboard → API Keys) and paste it into OnNow’s server token field.`
      );
    }
    ...
    while (true) {
      try {
        ...
        if (status === 401) {
          if (this._usesLegacyEmbyTokenQueryAuth()) {
            e.message += ` | ${this.appName}: HTTP 401 — wrong or revoked API key, or the key lacks access. Regenerate the key in the server dashboard and update OnNow. If you use a reverse proxy, ensure it forwards the X-Emby-Token header (OnNow also sends api_key on the query string).`;
          } else {
            e.message += ` | Jellyfin: HTTP 401 — invalid or revoked API key (Dashboard → API Keys), or the server blocked the request. OnNow uses Authorization: MediaBrowser (required on Jellyfin 10.11+ when legacy X-Emby-Token is disabled). Ensure your reverse proxy forwards the Authorization header if you use one.`;
          }
        }
    ...
    if (posterSyncFull) {
      ...
      if (
        ...
        posterMetadataDb.shouldSkipSyncItem(
          ...
          rawApiId,
          sourceUpdatedAt,
          { background: pullBackground, logo: pullLogo }
        )
      ) {
        if (sp) {
