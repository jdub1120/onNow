...
- These scripts will only work on a `Raspberry Pi 4/5`, that is being used to display OnNow
...
```
## Troubleshooting
- OnNow does not yet have its own dedicated support channel (the upstream PosterrX Discord — [Discord](https://discord.gg/AEhVjqX4Af) — and the original Posterr community — [Discord](https://discord.gg/TcnEkMEf9J) — are not OnNow support channels).
...
- The sleepwatch script polls the OnNow `/api/sleep` endpoint every 5 seconds to determine if OnNow is in scheduled sleep mode or not. It then issues an `on` or `standby` command to the attached display.

The script does the following:
- Gathers the URL for OnNow, current user and home directory
